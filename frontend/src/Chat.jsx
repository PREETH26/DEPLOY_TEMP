...
function Chat() {
  ...
  const [unreadCounts, setUnreadCounts] = useState({});
  const [lastViewed, setLastViewed] = useState({});
  const [selectedMessages, setSelectedMessages] = useState(new Set()); // Track selected messages
  ...
  const toggleMessageSelection = (msg) => {
    const chatId = getChatId(selectedChat);
    const messageKey = `${chatId}-${msg.timestamp}-${msg.content}`;
    const newSelectedMessages = new Set(selectedMessages);
    if (newSelectedMessages.has(messageKey)) {
      newSelectedMessages.delete(messageKey);
    } else {
      newSelectedMessages.add(messageKey);
    }
    setSelectedMessages(newSelectedMessages);
  };

  const handleCopySelected = () => {
    const selectedText = Array.from(selectedMessages).map(key => {
      const [chatId, timestamp, content] = key.split('-');
      return content;
    }).join('\n');
    navigator.clipboard.writeText(selectedText).then(() => {
      setMessage("Messages copied to clipboard!");
      setTimeout(() => setMessage(""), 3000);
      setSelectedMessages(new Set()); // Clear selection after copying
    }).catch(err => console.error('Failed to copy text: ', err));
  };

  const handleEditSelected = () => {
    // Commented out as endpoint is not ready
    // const selectedMsg = Array.from(selectedMessages)[0]; // For single selection editing
    // if (selectedMsg) {
    //   const [, , content] = selectedMsg.split('-');
    //   setInput(content);
    //   setSelectedMessages(new Set()); // Clear selection after editing
    //   // Implement backend endpoint and socket emit here
    // }
  };

  const handleDeleteSelected = () => {
    // Commented out as endpoint is not ready
    // const selectedMsgKeys = Array.from(selectedMessages);
    // if (selectedMsgKeys.length > 0) {
    //   selectedMsgKeys.forEach(key => {
    //     const [chatId, timestamp, content] = key.split('-');
    //     // Implement backend endpoint and socket emit here
    //   });
    //   setSelectedMessages(new Set()); // Clear selection after deletion
    // }
  };
  ...
  const handleChatSelect = (chat, type, classGroup = null) => {
    ...
    setFilePreview(null);
    if (fileInputRef.current) fileInputRef.current.value = "";
    setSelectedMessages(new Set()); // Clear selection when changing chat
  ...
  const handleShiftTabs = (event) => {
    ...
    if (event.key === "Tab") {
      ...
      setFilePreview(null);
      if (fileInputRef.current) fileInputRef.current.value = "";
      setSelectedMessages(new Set()); // Clear selection when switching tabs
  ...
  const handleTabClick = (chat) => {
    ...
    if (fileInputRef.current) fileInputRef.current.value = "";
    const chatId = getChatId(chat);
    setSelectedMessages(new Set()); // Clear selection when clicking tab
  ...
  const handleTabClose = (chat, e) => {
    ...
    if (selectedChat && getChatId(selectedChat) === chatId) {
      if (updatedOpenChats.length > 0) {
        ...
        setSelectedChat(lastChat);
        const lastChatId = getChatId(lastChat);
        setSelectedMessages(new Set()); // Clear selection when closing tab
        ...
        setFile(null);
        setFilePreview(null);
        setSelectedMessages(new Set()); // Clear selection when no chats
        localStorage.removeItem("selectedChatId");
        localStorage.removeItem("selectedChatType");
      }
    }
  ...
  const sendMessage = async () => {
    ...
    if (input.trim()) {
      const contentWithLineBreaks = input; // Preserve line breaks as entered
      if (selectedChat.type === "single") {
        socket.emit("send-message", { receiver: selectedChat.data._id, content: contentWithLineBreaks });
      } else if (selectedChat.type === "group") {
        socket.emit("send-group-message", { chatId: selectedChat.data.chat._id, content: contentWithLineBreaks });
      } else if (selectedChat.type === "subject") {
        socket.emit("send-group-message", { chatId: selectedChat.data.chat[0]._id, content: contentWithLineBreaks });
      }
      setInput("");
    }
  ...
  const handleInputChange = (e) => {
    ...
    if (hiddenTextareaRef.current) {
      hiddenTextareaRef.current.value = e.target.value;
      const height = hiddenTextareaRef.current.scrollHeight;
      messageRef.current.style.height = `${Math.min(height, 40)}px`;
    }
  };
  ...
  const handlePaste = (e) => {
    ...
    handleInputChange({ target: { value: input + text } });
  };
  ...
  const renderMessagesWithDates = () => {
    ...
    combinedMessages.forEach((item, index) => {
      ...
      if (item.type === "message") {
        ...
        const chatId = getChatId(selectedChat);
        const messageKey = `${chatId}-${msg.timestamp}-${msg.content}`;
        const isSelected = selectedMessages.has(messageKey);
        const isSender = selectedChat.type === "single" 
          ? (msg.senderId === profile._id && msg.receiver === selectedChat.data._id)
          : msg.senderId === profile._id;

        if (selectedChat.type === "single" && !isSender && !(msg.senderId === selectedChat.data._id && msg.receiver === profile._id)) return;

        messagesWithDates.push(
          <div
            key={index}
            className={`p-2 my-2 rounded-md w-fit max-w-[80%] sm:max-w-[50%] break-words relative ${
              isSender
                ? "sent bg-blue-500 text-white self-end ml-auto shadow-md"
                : "received bg-gray-300 text-black shadow-md"
            } ${isSelected ? "border-2 border-blue-500" : ""}`} // Highlight selected messages
            onClick={(e) => {
              if (e.detail === 2) toggleMessageSelection(msg); // Double-click to select
            }}
          >
            {selectedChat.type !== "single" && (
              <p className="font-bold mb-1 text-sm sm:text-base">
    ...
    // Render selection toolbar if messages are selected
    if (selectedMessages.size > 0) {
      messagesWithDates.push(
        <div className="fixed bottom-16 right-4 bg-gray-800 text-white p-2 rounded-md shadow-lg flex space-x-4">
          <button
            onClick={handleCopySelected}
            className="flex items-center space-x-2 hover:bg-gray-700 p-1 rounded"
          >
                ...
                <div className="flex mt-3 pb-2 pl-2 pr-2">
                  <div className="flex-1 flex flex-col">
                    ...
                    <textarea
                      ...
                      rows="1"
                      ref={messageRef}
                      style={{ height: "40px", maxHeight: "40px", lineHeight: "20px", paddingTop: "8px" }}
                    />
                    <textarea
