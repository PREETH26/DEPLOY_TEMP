...
function Chat() {
  ...
  const [unreadCounts, setUnreadCounts] = useState({});
  const [lastViewed, setLastViewed] = useState({});
  const [contextMenu, setContextMenu] = useState({ visible: false, x: 0, y: 0, message: null }); // For dropdown menu
  ...
  const handleChatSelect = (chat, type, classGroup = null) => {
    ...
    setFilePreview(null);
    if (fileInputRef.current) fileInputRef.current.value = "";
    setContextMenu({ visible: false, x: 0, y: 0, message: null }); // Clear context menu when changing chat
  ...
  const handleShiftTabs = (event) => {
    ...
    if (event.key === "Tab") {
      ...
      setFilePreview(null);
      if (fileInputRef.current) fileInputRef.current.value = "";
      setContextMenu({ visible: false, x: 0, y: 0, message: null }); // Clear context menu when switching tabs
  ...
  const handleTabClick = (chat) => {
    ...
    if (fileInputRef.current) fileInputRef.current.value = "";
    const chatId = getChatId(chat);
    setContextMenu({ visible: false, x: 0, y: 0, message: null }); // Clear context menu when clicking tab
  ...
  const handleTabClose = (chat, e) => {
    ...
    if (selectedChat && getChatId(selectedChat) === chatId) {
      if (updatedOpenChats.length > 0) {
        ...
        setSelectedChat(lastChat);
        const lastChatId = getChatId(lastChat);
        setContextMenu({ visible: false, x: 0, y: 0, message: null }); // Clear context menu when closing tab
        ...
        setFile(null);
        setFilePreview(null);
        setContextMenu({ visible: false, x: 0, y: 0, message: null }); // Clear context menu when no chats
        localStorage.removeItem("selectedChatId");
        localStorage.removeItem("selectedChatType");
      }
    }
  ...
  const renderMessagesWithDates = () => {
        ...
        messagesWithDates.push(
          <div
            ...
            className={`p-2 my-2 rounded-md w-fit max-w-[80%] sm:max-w-[50%] break-words relative ${
              (selectedChat.type === "single" ? (msg.senderId === profile._id) : (msg.senderId === profile._id))
                ? "sent bg-blue-500 text-white self-end ml-auto shadow-md"
                : "received bg-gray-300 text-black shadow-md"
            }`}
            onContextMenu={(e) => handleContextMenu(e, msg)} // Right-click to show dropdown
            onClick={(e) => {
              if (e.type === "click") handleContextMenu(e, msg); // Left-click to show dropdown
            }}
          >
            {selectedChat.type !== "single" && (
              <p className="font-bold mb-1 text-sm sm:text-base">
    ...
    if (contextMenu.visible && contextMenu.message) {
      messagesWithDates.push(
        <div
          key="context-menu"
          className="fixed bg-white border border-gray-300 shadow-lg rounded-md p-2 z-50"
          style={{ top: contextMenu.y, left: contextMenu.x }}
          onClick={(e) => e.stopPropagation()}
        >
          <ul className="text-sm">
            <li className="hover:bg-gray-100 p-1 rounded cursor-pointer" onClick={handleCopy}>
              <FaCopy className="inline mr-2" /> Copy
            </li>
            {/* Commented out as endpoint is not ready */}
            {/* <li className="hover:bg-gray-100 p-1 rounded cursor-pointer" onClick={handleEdit}>
              <FaEdit className="inline mr-2" /> Edit
            </li>
            <li className="hover:bg-gray-100 p-1 rounded cursor-pointer" onClick={handleDelete}>
              <FaTrash className="inline mr-2" /> Delete
            </li> */}
            <li className="hover:bg-gray-100 p-1 rounded cursor-pointer" onClick={closeContextMenu}>
              <IoMdClose className="inline mr-2" /> Cancel
            </li>
          </ul>
        </div>
      );
  ...
  return (
    <div className="min-h-screen overflow-clip bg-white" onClick={closeContextMenu}>
      <div className="flex items-start w-full">
        <div className="group relative">
          <img
            src={Logo}
