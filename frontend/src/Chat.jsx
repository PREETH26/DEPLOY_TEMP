...
import { HiMiniMagnifyingGlass } from "react-icons/hi2";
import { motion } from "framer-motion";
import { FaCopy, FaEdit, FaTrash } from "react-icons/fa"; // Added icons for copy, edit, and delete
...
function Chat() {
  ...
  const fileInputRef = useRef(null);
  const messageRef = useRef(null);
  const hiddenTextareaRef = useRef(null); // Ref for hidden textarea to measure content
  const isDarkMode = DarkMode();
  const dragItem = useRef(null);
  const [previewImageUrl, setPreviewImageUrl] = useState(null);
  const [previewFileName, setPreviewFileName] = useState(null);
  ...
  const handleInputChange = (e) => {
    ...
    // Sync hidden textarea content to measure height
    if (hiddenTextareaRef.current) {
      hiddenTextareaRef.current.value = e.target.value;
      const height = hiddenTextareaRef.current.scrollHeight;
      messageRef.current.style.height = `${Math.min(height, 40)}px`; // Cap at 40px
    }
  };
  ...
  const handleEdit = (message) => {
    ...
    // setInput(message.content); // For example, pre-fill the input with the message to edit
    // You would need to implement the backend endpoint and socket emit here
  };
            ...
            >
              ...
              <div className="absolute top-2 right-2 flex space-x-2">
                <button 
                  ...
                  className="text-white bg-gray-600 p-1 rounded-full hover:bg-gray-700"
                  title="Copy"
                >
                  <FaCopy />
                </button>
                ...
                {/* <button 
                  ...
                  className="text-white bg-yellow-500 p-1 rounded-full hover:bg-yellow-600"
                  title="Edit"
                >
                  <FaEdit />
                </button>
              ...
              <>
                ...
                <div className="flex mt-3 pb-2 pl-2 pr-2">
                  <div className="flex-1 flex flex-col">
                    ...
                    />
                    {/* Hidden textarea to measure content height */}
                    <textarea
