...
//   const [openChats, setOpenChats] = useState([]);
//   const [selectedChat, setSelectedChat] = useState(null);
//   const [chatMessages, setChatMessages] = useState({}); // Changed to object to store messages by chat ID
//   const [input, setInput] = useState("");
//   const [message, setMessage] = useState("");
//   const [file, setFile] = useState(null);
//   const [filePreview, setFilePreview] = useState(null);
//   const [uploadedFiles, setUploadedFiles] = useState({}); // Changed to object to store files by chat ID
//   const [isUploading, setIsUploading] = useState(false);
//   const [isPreviewOpen, setIsPreviewOpen] = useState(false);
//   const [activeSection, setActiveSection] = useState(() => {
//     return localStorage.getItem("activeSection") || "group";
...
//   // Helper to get chat ID based on chat type
...
//   useEffect(() => {
//     // Load initial chat history for all open chats
//     const fetchChatData = () => {
...
//     // Set up interval to fetch data every minute (60 seconds)
//     const interval = setInterval(fetchChatData, 60 * 1000); // 60 seconds = 60 * 1000 ms
...
//     dragItem.current = index; // Store the index of the dragged tab
...
//     if (draggedIndex === null || draggedIndex === dropIndex) return; // No change needed

//     const newOpenChats = [...openChats];
//     const [draggedChat] = newOpenChats.splice(draggedIndex, 1); // Remove dragged item
//     newOpenChats.splice(dropIndex, 0, draggedChat); // Insert at new position

//     setOpenChats(newOpenChats);
//     console.log("Dropped at index:", dropIndex, "New order:", newOpenChats);
//     dragItem.current = null; // Reset dragged item
//   };
...
//     if (openChats.length === 0) return; // No chats to switch

//     // Use getChatId to find the current chat by ID, not reference
...
//       if (event.key === "Tab") {
//         event.preventDefault(); // Prevent default behavior for both
...
//                   }`}
//                   onClick={() => handleTabClick(chat)}
//                   draggable={true} // Make the tab draggable
