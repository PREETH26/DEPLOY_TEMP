...
//   // Separate useEffect for WebSocket setup to minimize re-renders
//   useEffect(() => {
//     const fetchChatData = () => {
//       if (!selectedChat) return;
//       const chatId = getChatId(selectedChat);
...
//     fetchChatData();
//     // Reduced polling to every 5 minutes (optional, can remove entirely if WebSocket is reliable)
...
//       // Update chatMessages for all chats, not just the selected one
//       setChatMessages((prev) => {
//         const existingMessages = prev[chatId] || [];
//         const updated = existingMessages.some(
//           (m) => m.timestamp === message.timestamp && m.content === message.content
...
//       socket.off("error-message");
//     };
//   }, [profile, selectedChat, lastViewed, subjectGroups, groupChats]); // Stable dependencies

//   // Ensure UI scrolls to new messages
//   useEffect(() => {
//     if (newMessage.current) {
//       newMessage.current.scrollIntoView({ behavior: "auto" });
//     }
...
function MobileChat() {
  ...
  // WebSocket effect dependent on profile to ensure it’s available
  useEffect(() => {
    if (!profile) return; // Wait for profile to be set
    ...
    fetchChatData();
    const interval = setInterval(fetchChatData, 300 * 1000); // Optional polling every 5 minutes
